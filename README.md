...
1. **Clone the repository to your local machine:**
    ...
    ```sh
    git clone https://github.com/lizabethwordp/Grafana-Dashboard.git
    ```
...
3. **Customize Variables (Optional):** You can modify variables in `variables.tf` in the default block or create a `terraform.tfvars` file to override defaults.
...
After Terraform provisions the infrastructure, Grafana and MySQL are automatically set up on the EC2 instance using Docker with the `data.sh` script. The `data.sh` file is referenced using the `user_data` command declaration in the Grafana instance provisioning as seen below:
